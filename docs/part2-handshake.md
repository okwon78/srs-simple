...
```text
S2 = 받은 C1의 1536바이트를 그대로 복사
C2 = 받은 S1의 1536바이트를 그대로 복사 (관례) — 서버는 읽고 버린다
```
...
```text
심플의 C1:   time(4) | version(4) | random(1528)
복잡의 C1:   time(4) | version(4) | [ key 블록(764) | digest 블록(764) ]   ← schema0
        또는 time(4) | version(4) | [ digest 블록(764) | key 블록(764) ]   ← schema1
```

- **key 블록(764B)**: DH 공개키 128바이트가 랜덤 패딩 속 가변 오프셋에 숨어 있다
  (블록 마지막 4바이트가 오프셋).
- **digest 블록(764B)**: HMAC-SHA256 다이제스트 32바이트가 역시 가변 오프셋에 숨어 있다.
  다이제스트는 "자기 자신을 뺀 1,504바이트"에 대해 계산한다.
- 두 블록의 순서가 **schema0(key-digest) / schema1(digest-key)** 두 가지라서, 서버는
  둘 다 시도해 봐야 한다. 어느 쪽인지 알려 주는 필드는 없다.
...
복잡판(원본 `srs_protocol_rtmp_handshake.cpp:1152`)은 C1을 schema0으로 파싱해 다이제스트를
검증하고, 실패하면 schema1로 재시도하고, 그래도 실패하면 — 즉 상대가 다이제스트를 넣지 않은
평범한 랜덤을 보냈으면 — `ERROR_RTMP_TRY_SIMPLE_HS`라는 **제어 흐름용 에러**를 반환한다.
이 에러를 받은 바깥 계층이 심플판으로 갈아탄다. §3에서 본 `SrsHandshakeBytes`의 멱등성
가드가 바로 이 갈아타기를 가능하게 하는 장치다: 복잡판이 소켓에서 이미 꺼낸 C0C1을 심플판이
버퍼에서 재사용한다.
