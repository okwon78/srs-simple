...
- **메시지 스트림**은 애플리케이션이 실제로 다루는 단위다. "connect 해라", "이것은
  타임스탬프 1000ms의 비디오 프레임이다" 같은 논리적 의미는 전부 메시지 층에 있다.
...
두 축 가운데 csid 축은 Part 3(청크 재조립)에서, sid 축은 Part 6(createStream이 sid를
발급하는 순간)에서 각각 자세히 다룬다.
...
1. **핸드셰이크부터 connect 부트스트랩까지는 publisher와 player가 동일하다.** 서버는 그 다음에
   오는 첫 커맨드를 보고서야 상대의 의도를 안다 — releaseStream이면 그 즉시 publisher(Flash Media Live Encoder 스타일)로
   확정하고, createStream이면 그 다음 커맨드(play냐 publish냐)까지 기다린다. 이 판별이 Part 6의
   `identify_client`다.
...
지금까지 그림 두 장을 그렸다. §2의 **층 모델**(TCP → 청크 → 메시지)과 §3의
**수명주기**(핸드셰이크 → 협상 → 미디어 반복). 이후 파트들은 이 두 그림을 나눠 갖는다 —
먼저 층을 아래에서 위로 하나씩 오르고(Part 2~5), 그다음 시간 순서로 세션을 따라 걷고(Part 6~8),
마지막으로 프로토콜 밖으로 나간다(Part 9~11):

```mermaid
flowchart TB
    subgraph G1["층을 아래에서 위로 — §2의 층 모델"]
        direction LR
        P2["Part 2<br/>핸드셰이크"] --> P3["Part 3<br/>청크 스트림"] --> P4["Part 4<br/>컨트롤 메시지"] --> P5["Part 5<br/>AMF0"]
    end
    subgraph G2["시간 순서대로 — §3의 수명주기"]
        direction LR
        P6["Part 6<br/>connect·식별"] --> P7["Part 7<br/>publish"] --> P8["Part 8<br/>play"]
    end
    subgraph G3["프로토콜 밖으로"]
        direction LR
        P9["Part 9<br/>서버 내부"] --> P10["Part 10<br/>HLS"] --> P11["Part 11<br/>LL-HLS"]
    end
    G1 --> G2 --> G3
```

**층을 아래에서 위로 (Part 2~5)** — §2 층 모델의 각 층이 파트 하나씩이다:

- **Part 2 (핸드셰이크)**: 층 모델이 등장하기 전, 연결 직후 가장 먼저 오가는 고정 크기 바이트
  교환 — TCP 바로 위의 서막이다. 왜 하필 3,073바이트를 주고받는지, Flash 시절의 암호화 검증이
  왜 지금은 형식만 남았는지 다룬다.
- **Part 3 (청크 스트림)**: 가운데 층의 전부. 청크 헤더 fmt 0~3, extended timestamp,
  재조립 상태 기계. 세션 바이트의 99%가 청크 층을 지나며, **이 시리즈에서 가장 밀도 높은 파트다.**
- **Part 4 (컨트롤 메시지)**: 청크 층 자체를 관리하는 type 1~6 메시지들. "SetChunkSize를
  언제 보내야 하는가" 같은 타이밍 규칙.
- **Part 5 (AMF0)**: 맨 위층인 메시지 층에 진입 — 커맨드 메시지의 페이로드 인코딩이다.
  connect의 실제 바이트를 손으로 읽는다.

**시간 순서대로 (Part 6~8)** — §3의 시퀀스 다이어그램을 구간별로 확대한다:

- **Part 6 (접속과 식별)**: connect 부트스트랩, 그리고 첫 커맨드로 publisher/player를
  판별하는 `identify_client`.
- **Part 7 (송출)**: publish 협상부터 오디오/비디오 메시지가 흘러들어오는 미디어 루프까지.
- **Part 8 (시청)**: play 협상과, 방송 중간에 들어온 플레이어가 즉시 화면을 받는
  중간 입장 문제.

**프로토콜 밖으로 (Part 9~11)** — 네트워크 반대편, 서버 내부와 HTTP 배포:

- **Part 9 (서버 내부)**: 받은 미디어를 N명의 플레이어에게 무복사로 뿌리는 서버 설계 —
  팬아웃, GOP·메타 캐시, 지터 보정.
- **Part 10 (HLS)**: 같은 스트림을 TS 세그먼트와 m3u8로 바꿔 HTTP로 배달하는
  트랜스먹싱 경로. §1 그림의 배포 구간이다.
