...
이 글은 Part 2까지 읽었다고 가정한다.
핸드셰이크(Part 2)가 끝난 소켓에는 클라이언트의 첫 청크(connect 커맨드의 첫 조각)가
...
이 다섯 함수([srs_protocol_rtmp_stack.cpp:219-1094](../src/protocol/srs_protocol_rtmp_stack.cpp#L219-L1094)의
`:219/616/714/773/1050`)가 이 글의 뼈대다. 원본 SRS에서도 함수 이름과 구조가 완전히 같다
...
RTMP의 답은 **차등 인코딩(differential encoding)** 이다. 같은 흐름(csid)의
연속된 메시지는 헤더가 거의 같다는 데서 출발한다. 오디오 스트림이라면 type은 늘 8이고, stream_id는 늘 1이고,
...
극단이 fmt=3이다. 헤더가 basic header 1바이트로 끝난다. FMLE(Flash Media Live Encoder)가 보내는 연속 오디오 프레임은
메시지 하나가 통째로 `0xC4` 1바이트 + 페이로드다 — 11바이트짜리 헤더가 1바이트로 줄었다.

여기서 청크 층의 핵심 성질이 나온다: **차등 인코딩은 수신자가 상태를 유지해야만 성립한다.**
"직전과 같음"이 성립하려면 수신자가 csid별로 직전 헤더를 기억하고 있어야 한다. 그 기억이
`SrsChunkStream`이고([srs_protocol_rtmp_stack.hpp:349-375](../src/protocol/srs_protocol_rtmp_stack.hpp#L349-L375), §3.2),
청크 스택이 "상태 기계"인 이유다.
...
첫 바이트만 보면 청크의 정체를 읽을 수 있게 된다. 아래 표는 새 규칙이 아니라 방금의 fmt/csid
파싱을 실제 바이트 값에 적용해 본 **해독 예시** 다. 이 시리즈와 테스트 코드의 hex 덤프에 반복해서
나오는 값들을 모은 치트시트다. 단, "정체" 열은 아직 설명하지 않은 내용이다 — csid 2는 컨트롤,
3은 커맨드, 4는 미디어에 쓴다는 관례인데, Part 4·6에서 자세히 다루므로 지금은 "이런 배정이
있구나" 정도만 알고 넘어가면 된다:
...
반환된다. 즉 **호출자는 청크의 존재를 전혀 모른다.** `recv_message`의 서명이 청크 층의 추상화
