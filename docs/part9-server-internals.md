...
Part 1~8은 전부 네트워크 위의 이야기였다 — 어떤 바이트가 어떤 순서로 오가는가.
이번 파트는 네트워크를 벗어난다. OBS 한 대가 publish하는 스트림을 플레이어 1,000명이
...
같은 발상이 RTMP 밖에서 한 번 더 쓰인다. LL-HLS의 파트(0.5초 fMP4 조각)도
`std::shared_ptr`로 공유돼, HTTP 스레드가 락 안에서는 포인터만 받고 소켓 쓰기는 락
밖에서 한다 — 손수 만든 refcount(`SrsSharedPtrPayload`, 원본이 C++98 시절에 쓴
방식)와 표준 라이브러리의 차이일 뿐 "바이트는 한 벌, 참조만 늘린다"는 원리는 같다
([Part 11 §6.1](part11-llhls.md)).
...
(`last_pkt_time`/`last_pkt_correct_time`)가 공유되면 한 플레이어의 타임라인이 다른 플레이어를
