// srs_simple — 원본: trunk/src/protocol/srs_protocol_rtmp_stack.cpp
// 1부(S5): 청크 스트림 스택. 2부(S6): 커맨드 패킷 + SrsRtmpServer.
...
srs_error_t SrsProtocol::do_send_messages(SrsSharedPtrMessage **msgs, int nb_msgs)
{
    ...
    for (int i = 0; i < nb_msgs; i++)
    {
        ...
        while (p < pend)
        {
            ...
            // always has header
            // 첫 청크는 fmt=0(c0), 이어지는 청크는 fmt=3(c3) — CLAUDE.md §2.2.
...
srs_error_t SrsProtocol::do_decode_message(SrsMessageHeader &header, SrsBuffer *stream, SrsPacket **ppacket)
{
    ...
    if (header.is_amf0_command() || header.is_amf3_command() || header.is_amf0_data() || header.is_amf3_data())
    {
        ...
        // 원본은 그 밖의 커맨드를 SrsCallPacket(RPC 일반형)으로 받아 null 응답하지만,
        // closeStream/pause/_checkbw 등은 라이브 최소 경로에 불필요해 드롭한다 — CLAUDE.md §5.6.
...
srs_error_t SrsRtmpServer::identify_client(int stream_id, SrsRtmpConnType &type, string &stream_name, srs_utime_t &duration)
{
    ...
    while (true)
    {
        ...
        // 원본은 SrsCallPacket(_checkbw 등)에 null 응답을 보내지만, Call 패킷은 제거 — CLAUDE.md §5.6.
...
srs_error_t SrsRtmpServer::start_fmle_publish(int stream_id)
{
    ...
    // 주의: onStatus(NetStream.Publish.Start)는 여기서 보내지 않는다.
    // 인가(훅) 통과 후 start_publishing이 분리 송신 — 이슈 #4037, CLAUDE.md §5.3.
